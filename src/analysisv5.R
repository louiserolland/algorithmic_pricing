...
# KEY FIX: use Z_hq as a continuous instrument.
# Binarising at the median (previous approach) discards variation and
# creates compliance score inversions (defiers) after conditioning on X,
# because the binary instrument has near-zero residual variation in some
# leaves — making the LATE denominator unstable and ATE estimates blow up.
# Using the raw continuous Z_hq preserves full instrument variation.
#
# Additional stabilisation:
#   min.node.size = 20  — larger leaves ensure enough observations for a
#                          stable within-leaf first stage
#   tune.parameters = "none" — auto-tuning can minimise prediction error
#                               by creating small leaves, worsening weak-IV
cat("Z_hq summary:\n"); print(summary(Z))

Z_cont    <- matrix(Z, ncol = 1)   # continuous instrument, no binarisation

fs_check  <- lm(D ~ Z + n + delta + mu + alpha_lr, data = df)
fs_f_stat <- summary(fs_check)$fstatistic[1]
cat(sprintf("First-stage F (Z_hq continuous, on D | X): %.2f", fs_f_stat))
if (fs_f_stat < 10) {
...
iv_forest <- instrumental_forest(
  ...
  num.trees     = 2000,
  honesty       = TRUE,
  min.node.size = 20,        # stable within-leaf first stage
  tune.parameters = "none",  # prevent auto-tuning to small leaves
  seed          = 42
)

# Compliance score diagnostic — should all be positive (no defiers)
compliance <- iv_forest$W.orig - iv_forest$W.hat
cat(sprintf("Compliance score range: [%.4f, %.4f]\n",
            min(compliance), max(compliance)))
if (min(compliance) < 0) {
  cat("  WARNING: negative compliance scores remain — interpret ATE with caution\n")
} else {
...
page1 <- (p2 + p4) / (p5 + p6)
ggsave(file.path(results_dir, "cate_analysis.png"), page1, width = 14, height = 10, dpi = 180)
cat("  Saved: ../results/cate_analysis.png\n")

# Page 2: Chernozhukov et al. heterogeneity ladder (GATES + CLAN)
page2 <- p7 + p8
ggsave(file.path(results_dir, "gates_clan.png"), page2, width = 14, height = 5, dpi = 180)
cat("  Saved: ../results/gates_clan.png\n")

# Individual saves
ggsave(file.path(results_dir, "ate_comparison.png"),  p5, width = 8,  height = 5,  dpi = 180)
cat("  Saved: ../results/ate_comparison.png\n")

ggsave(file.path(results_dir, "cate_by_delta.png"),   p2, width = 8,  height = 5,  dpi = 180)
cat("  Saved: ../results/cate_by_delta.png\n")

ggsave(file.path(results_dir, "cate_heatmap.png"),    p4, width = 6,  height = 5,  dpi = 180)
cat("  Saved: ../results/cate_heatmap.png\n")

ggsave(file.path(results_dir, "finite_sample.png"),   p3, width = 7,  height = 5,  dpi = 180)
cat("  Saved: ../results/finite_sample.png\n")

ggsave(file.path(results_dir, "cate_scatter.png"),    p1, width = 9,  height = 5,  dpi = 180)
cat("  Saved: ../results/cate_scatter.png\n")
