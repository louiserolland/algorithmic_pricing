...
fs_results <- lapply(sample_sizes, function(n_sub) {
  ...
  # GRF at this sample size
  tryCatch({
    cf_sub <- instrumental_forest(
      sub_X, sub_Y, sub_D, sub_Z,
    ...
    list(n_sub=n_sub, ate_grf=NA, se_grf=NA,
         ate_ols=ate_ols_sub, se_ols=se_ols_sub, cate_var=NA)
  })
})
...
fs_long <- fs_df %>%
  ...
  mutate(method = ifelse(method == "se_grf", "GRF IV forest", "OLS interactions"))

p3 <- ggplot(fs_long, aes(x = n_sub, y = se, colour = method)) +
  geom_line(linewidth = 1.1) +
  geom_point(size = 2.5, stroke = 0.3, fill = "white", shape = 21) +
  scale_colour_manual(values = c("GRF IV forest" = clr_grf,
  ...
  labs(
    ...
    subtitle = "GRF needs more data to match OLS precision — the bias-variance trade-off.",
    x        = "Sample size  N  (log scale)",
    y        = "SE of ATE estimate",
    caption  = "OLS is efficient under correct specification; GRF pays a variance tax for flexibility."
  ) +
  theme_clean()
