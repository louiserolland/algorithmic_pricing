...
D is not randomly assigned because of market structures and therefore it is interesting 
to study the results of our ML methods for causal inference because they can capture things that OLS can't.
...
The intrument comes from Assad et al (2024). An example of this instrument is in the gasoline market. 
In his paper, Assad analyses that decisions that come from headquarters of a brand regarding algorithmic adoption, impact 
the brand other stations. For example if Shell has 50 petrol stations in Germany, you study station i in Munich;
The instrument is what fraction of Shell's other 49 stations (in Berlin, Frankfurt...) have adopted algorithmic pricing.


By leaving-it-out, we ensure that Zi is a property of other markets, it gives independence from market i's own adoption decision.'
By sharing the same unobserved HQ-level shock, this is a good indicator wheter you will adopt algorithmic pricing or not as well.
Moreover, if Munich's adoption has no direct effect on Frankfurt profit margin.
Frankfurt's profit are more likely determined by its local market structure. 

The instrument reflects the HQ level, wheter a central decision propagates across the network. And this decision
is to a certain extent orthogonal to a specific's station local competitive environment.

Output: simulation_data_v4.csv — ready for R analysis

Usage:
    python simulatev4_fin.py            # very long run
    python simulatev4_fin.py --fast     # Quick test 
"""
...
def logit_demand(prices, a, mu, a0):
    ...
    Each consumer picks the product with highest utility. 
    The multinomial market share formula, each firm's share is its exponential utility relative to
    the sum across all options including the outside good.
    """
...
def monopoly_price(c_val, a_i, mu, a0, n):
    ...
    # here we add a bound to prevent numerical issues near marginal cost.
    return res.x
...
def nash_price(c_val, a_i, mu, a0, n):
    ...
    This represents the pure competitive behavior
    """
    c = np.full(n, c_val)
    a = np.full(n, a_i)
...
# Because we compute the monoply and Nash outcome, we can then compute our outcome variable
# delta. We compute the fraction of monoply surplus firms achieve above Nash. 
# A delta of 0 means purely competitive, a delta of 1 means full cartel collusion.
...
def simulate_qlearning(n, delta, mu, alpha_lr, beta_exp,
    ...
    It controls the decay rate of the exploration probability.
    We wanted a more thorough exploration before policy convergence so that we have a cleaner, more reliable final 
    equilibria.
    It reduces the risk that observed collusion is a premature convergence.
    ...
    - Q_0 initialised to discounted payoff under opponent uniform randomisation
      ...
      The intuition behind this idea is that before learning anything, what would firm
      i's expected discounted payoff be if it choose action a and all rivals
      randomised uniformly over the price grid.
    ...
    In calvano's setting, he says that 2M iterations almost all runs converge, but that was not the case for my simulation.
    Therefore I went with 5M iterations. (Note: it takes a very long time to run, about 6 hours to run)

    - Delta recorded over last 100,000 periods after convergence or at end.

    We want to measure profits once agents have figured out how to play.
    And because the learned strategy is not a fixed price, even at full convergence,
    firms are playing dynamically and the 100 000 period average captures this cycle.  
    We use this number because convergence is defined as 100 000 post-convergence periods.  
    
    Returns: (Delta, avg_price, converged)
        Delta = (avg_pi - pi_N) / (pi_M - pi_N)  in [0,1]
        
    Each session gets its own seeded random generator so that results are reproducible
    but independent across sessions. A session is one complete simulation run for 
    a single bundle of market parameters.
    
    The action space for each agent is a discrete price grid of 15 equally spaced points.
    
    It spans below Nash to slightly above monopoly.
    """
    ...
    # the state that firm i observes is rival's prices from the previous period.
    n_states = m ** n

    # --- Calvano Q initialisation (equation 8) ---
    # Q_i0(s, a_i) = E[pi_i | a_i, a_{-i} ~ Uniform] / (1 - delta)
    # = sum_{a_{-i}} pi_i(a_i, a_{-i}) / ((1-delta) * m^{n-1})
    # By dividing with (1-delta), we get a discounted present value.
    # This is the intuition of Q-value: the total discounted future payoff from taking action a in state s.
    ...
    # Here we check if the  policy (argmax Q) has stabilised for 100,000 consecutive periods.
...
def simulate_myopic(n, mu, c_val, a_i, a0, n_iter, seed):
    ...
    No collusion because firms don't consider future punishment, so no incentive to deviate from static best-response.

    This is simmilar to Cournot-style tatonnement on prices.
...
def adoption_prob(n, mu, delta, gamma, chain_shock=0.0):
    ...
    It computes the conditional probability of algorithmic pricing adoption.
    It computes it for one firm and takes into account its market-level covariates
    and the unobserved chain-level shock.

    For n we take -2 because this is the baseline number of firms (duopoly).
    0.25 is the lower end of the demand intercept range in the grid.
    0.95 is the lower end of the discount factor range in the grid.

    The DGP uses a discrete grid. However, the GRF needs variation within leaves
    to split. And this variation reflects the real-world fact that firms with similar market structures may still differ in their adoption decision due to unobserved factors (e.g. managerial attitudes, local competitive dynamics, etc.).

    Therefore, what is added is Gaussian noise to each observation's covariate value before storing them.

    It is important to note that the chain instrument is assigned based on the original 
    grid not on the jittered value so that the instrument validity and its orthogonality 
    is preserved.

    Economic motivation (Assad et al. 2024):
...
def build_sessions(config):
    ...
    Each combinations of grid values is randomly mapped to one of 10 chains.
    Each chain draws a persistent unobserved shock from a normal distribution.
    
    It answers the following question: for each market that exist in the dataset, what 
    are its characteristics and what treatment dit it receive?

    Chain_shock is somethi,ng that influences adoption but is neither a market-level covariate
    or a profit determinant.
...
def add_instrument(df):
    ...
    It answers the question: given that chain shocks exist and influence adoption, how could someone 
    detect their effect without observing them directly? 
    """
