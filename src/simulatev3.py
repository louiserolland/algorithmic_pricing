...
if __name__ == '__main__':
    ...
    if FAST_MODE:
        ...
        config = dict(
            ...
            alpha_vals = [0.15],
            beta_exp   = 4e-6,
            n_sessions = 5,           # sessions per grid cell
            ...
            # Chain structure: n_chains random chains crossing covariate cells.
            # With n_chains=10, each chain has ~(total_sessions/10) observations,
            # giving genuine within-leaf Z variation for the IV forest.
            n_chains = 10,

            # Binary HQ mandate shift: log-odds shifted by ±chain_shock_sd.
            # With 1.5, mandate moves adoption prob by ~15-25 pp -> strong F-stat.
            chain_shock_sd = 1.5,
        ...
        config = dict(
            ...
            beta_exp   = 4e-6,
            n_sessions = 30,          # sessions per grid cell (~1620 total obs)
            ...
            # Chain structure: 10 random chains crossing covariate cells.
            # With n_sessions=30 and ~1620 obs, each chain has ~162 observations,
            # ensuring adequate within-leaf first-stage power for the IV forest.
            n_chains = 10,

            # Binary HQ mandate shift: ±chain_shock_sd on adoption log-odds.
            # Value 1.5 gives a ~20pp adoption gap between mandate=1 and mandate=0,
            # targeting parametric first-stage F >> 10 and stable IV forest leaves.
            chain_shock_sd = 1.5,
