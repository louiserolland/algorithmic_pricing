...
Usage:
    python simulatev2.py            # Full overnight run
    python simulatev2.py --fast     # Quick test (~5 min, small grid)
"""
...
def run_session(args):
    ...
    """
    (session_id, n,
     delta_obs, mu_obs,          # jittered: used for DGP + stored as X
     alpha_lr, beta_exp,
    ...
    return {
        ...
        'chain_id':   chain_id,
        'n':          n,
        'delta':      delta_obs,   # jittered value — what R sees
        'mu':         mu_obs,      # jittered value — what R sees
        'alpha_lr':   alpha_lr,
        'beta_exp':   beta_exp,
        'D':          D,
        'Delta':      Delta,
...
def build_sessions(config):
    ...
    # Assign chain_ids from GRID values — jitter does not affect chain membership.
    # This preserves instrument validity: Z_hq reflects chain-level adoption
    # shocks, not local parameter draws.
    chain_keys = list(product(delta_vals, mu_vals, alpha_vals))
    chain_id_map = {k: i for i, k in enumerate(chain_keys)}

    # Draw one unobserved HQ-level shock per chain.
    # This creates genuine within-chain correlation in D that is NOT
    # explained by local X = (n, delta, mu, alpha_lr), giving the
    # leave-one-out instrument Z_hq real first-stage power.
    # The shock does NOT enter the profit equation -> exclusion restriction holds.
    chain_shocks = {i: float(rng.normal(0, sigma_chain))
    ...
    for s in range(S):
        for n, delta, mu, alpha_lr in product(n_vals, delta_vals, mu_vals, alpha_vals):
            ...
            # --- Per-session continuous jitter ---
            # Each replicate of the same grid cell gets slightly different
            # parameters, giving GRF genuine continuous variation to split on.
            # Clipped to keep parameters in economically valid ranges.
            delta_obs = float(np.clip(
                delta + rng.normal(0, jitter.get('delta', 0.0)),
                0.50, 0.99   # discount factor: well inside (0,1)
            ))
            mu_obs = float(np.clip(
                mu + rng.normal(0, jitter.get('mu', 0.0)),
                0.05, 1.0    # substitutability: must be positive
            ))
            alpha_obs = float(np.clip(
                alpha_lr + rng.normal(0, jitter.get('alpha', 0.0)),
                0.01, 0.50   # learning rate: in (0, 0.5)
            ))

            # --- Confounded treatment assignment (with chain shock) ---
            # Uses jittered parameters — these are the true local conditions.
            cid = chain_id_map[(delta, mu, alpha_lr)]
            ...
            sessions.append((
                session_id, n,
                delta_obs, mu_obs,   # jittered — passed to run_session and stored
...
def run_pipeline(config):
    ...
    print(f"  {df['Z_hq'].corr(df['D']):.3f}  (should be > 0.3 after chain-shock fix)")
...
if __name__ == '__main__':
    ...
    if FAST_MODE:
        ...
        config = dict(
            ...
            # Chain-level HQ shock SD: creates within-chain D correlation
            # that is unexplained by local X -> gives Z_hq first-stage power.
            # Target: first-stage F > 10.  Value 0.8 delivers F ~ 15-25.
            chain_shock_sd = 0.8,

            # Jitter: continuous variation within each grid cell so GRF can
            # find real splits. SD chosen so ~95% of draws stay within 1 grid
            # step of the centre (grid step = 0.25 for delta, 0.15 for mu).
        ...
        config = dict(
            ...
            # Chain-level HQ shock: same as fast mode
            chain_shock_sd = 0.8,

            # Jitter: continuous variation within each grid cell.
            # Larger SD than fast mode because n_sessions=20 gives more
            # replicates per cell, so we can afford slightly wider jitter
            # without losing coverage of the grid centres.
